...
function launchParticlesJS(tag_id, params){
  ...
  pJS.fn.particle = function(color, opacity, position){
    ...
    this.draw = function(){
      ...
      switch(pJS.particles.shape){
        ...
        case 'dodec':
          pJS.canvas.ctx.beginPath();
      		pJS.canvas.ctx.moveTo(12+this.x,6+this.y);
      		pJS.canvas.ctx.lineTo(11+this.x,9+this.y);
      		pJS.canvas.ctx.lineTo(9+this.x,11+this.y);
      		pJS.canvas.ctx.lineTo(6+this.x,12+this.y);
      		pJS.canvas.ctx.lineTo(3+this.x,11+this.y);
      		pJS.canvas.ctx.lineTo(1+this.x,9+this.y);
      		pJS.canvas.ctx.lineTo(0+this.x,6+this.y);
      		pJS.canvas.ctx.lineTo(1+this.x,3+this.y);
      		pJS.canvas.ctx.lineTo(3+this.x,1+this.y);
      		pJS.canvas.ctx.lineTo(6+this.x,0+this.y);
      		pJS.canvas.ctx.lineTo(9+this.x,1+this.y);
      		pJS.canvas.ctx.lineTo(11+this.x,3+this.y);
      		pJS.canvas.ctx.closePath();
      	break;
      }
  ...
  pJS.fn.vendors.interactivity.grabParticles = function(p1, p2){
    ...
    /* Check distance between 2 particles + Checck distance between 1 particle and mouse position */
